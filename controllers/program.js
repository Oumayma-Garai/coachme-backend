...
const { index } = require("../models/Goal");
...
const getById = async (req, res) => {
  try {
    ...
    const similar_programs = await Program.find();
    const index_programs = [];
    for (let i = 0; i < similar_programs.length; i++) {
      if (
        program.goal == similar_programs[i].goal ||
        program.level == similar_programs[i].level
      ) {
        if (program._id !== similar_programs[i]._id) {
          index_programs.push(similar_programs[i]);
        }
      }
    }

    console.log(index_programs);
    if (!program)
      return {
        status: false,
        code: 404,
    ...
    res.send({ program, index_programs });
  } catch (err) {
    console.log("err", err);
  }
};
